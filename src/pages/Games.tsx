...
export default function Games() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-slate-900 via-emerald-900 to-slate-900">
      ...
      {/* Spline Background - Fixed position and cropped */}
      ...
      {/* Game Cards - Added mt-32 to shift down */}
      <div className="relative z-10 p-8 pt-0 mt-[290px]">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {games.map((game) => (
            ...
            >
              <div className="absolute inset-0 bg-gradient-to-r from-emerald-500/10 to-slate-500/10 
                opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-4">
                  <div className="p-3 bg-white/20 rounded-lg">
                    <game.icon className="w-6 h-6 text-emerald-400" />
                  </div>
                  <div className="flex items-center space-x-2">
                    <Zap className="w-5 h-5 text-emerald-400" />
                    <span className="text-emerald-400 font-semibold">{game.reward}</span>
                  </div>
                </div>
